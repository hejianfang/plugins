/* 如开启键盘控制事件后，相对应功能控制按键如下 按键 功能 w 放大 s 缩小 a 上一张
d 下一张 q 逆时针旋转 e 顺时针旋转 r 图片复位 esc 关闭图片预览 字段 值 备注 url
图片的本地或者线上地址，多图预览时可不传 预览的图片地址，多图预览时省略
mainBackground #fff 或者 rgba(255,255,255,.1) 整体背景颜色（可选） controlColor
#fff 或者 rgba(255,255,255,.1) 控制条字体颜色（可选） controlBackground #fff
或者 rgba(255,255,255,.1) 控制条背景颜色 （可选） closeColor #fff 或者
rgba(255,255,255,.1) 关闭图标的颜色 （可选） multiple 布尔值 true/false
是否多图预览 nowImgIndex Number 格式，如默认展示第二张，传入 1
多图预览时默认显示的图片下标 imgList Array 格式 [url1, url2, url3]
多图预览时传入的数组 keyboard 布尔值 true/false 是否开启键盘控制 */
<template>
  <transition name="fade">
    ...
    >
      <div class="he-img-wrap">
        <img src="@/assets/imgs/loading.gif" v-show="imgState === 1" />
        ...
        >
          <div class="iconfont icon-suoxiao" @click="scaleFunc(-0.15)">
            &#xe65e;
          </div>
          <div class="iconfont icon-fangda" @click="scaleFunc(0.15)">
            &#xe65d;
          </div>
          <!--          <div class="he-control-btn iconfont" v-show="isFull" @click="imgToggle">&#xe698;</div>-->
          <!--          <div class="he-control-btn iconfont" v-show="!isFull" @click="imgToggle">&#xe86b;</div>-->
          <div class="iconfont icon-nishizhen" @click="rotateFunc(-90)">
            &#xe670;
          </div>
          <div class="iconfont icon-shunshizhen" @click="rotateFunc(90)">
            &#xe66f;
          </div>
        </div>
        <div
